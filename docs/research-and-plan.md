...
排序（按用户后续反馈修订）：使用窗口缩略图外接矩形的中心作为旋转中心，按缩略图中心的角度顺时针排序；同角度按半径排序，仍相同则按窗口 ID 稳定排序。打开前记录最近窗口顺序，用 `wid` 找到最近使用的其他窗口作为初始项，最近使用顺序不参与后续导航排序。首次 Tab 只触发打开；之后的 Tab / 反引号分别顺时针 / 逆时针，包括打开期间缓存的输入。本次会话冻结顺序。若无法关联最近窗口，则使用起始窗口或空间顺序第一项，不按标题模糊匹配。
