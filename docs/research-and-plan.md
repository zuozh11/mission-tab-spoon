...
排序（按用户后续反馈修订）：窗口按从左到右、从上到下的逐行顺序循环；以缩略图中心及半个最小高度容差识别同行，行内按中心横坐标排序，重合时按窗口 ID 稳定排序。打开前记录最近窗口顺序，用 `wid` 找到最近使用的其他窗口作为初始项，最近使用顺序不参与后续导航排序。首次 Tab 只触发打开；之后的 Tab / 反引号分别按逐行顺序 / 完整逆序导航，包括打开期间缓存的输入。本次会话冻结顺序。若无法关联最近窗口，则使用起始窗口或空间顺序第一项，不按标题模糊匹配。
