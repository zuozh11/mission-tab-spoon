...
- **未分组窗口标识**：关闭“使窗口按应用程序成组”时，为每个缩略图补上带轻微阴影的应用图标，居中放在底边；系统手势打开调度中心时也会显示。
- **当前屏幕导航**：只选择鼠标所在屏幕上、调度中心实际展示的窗口；按窗口 ID 排除辅助功能列表中仍存在、但屏幕上不可见的其他空间窗口，导航与遮罩共用这一筛选。
...
窗口按**自适应分列、列内从上到下**循环。插件结合整屏窗口中心的横向间距与缩略图的典型宽度识别列：明显分成几列时优先走完当前列；横向均匀错开的窗口则保持从左到右。小幅横向偏移不会直接颠倒上下顺序；初步分列限制整体跨度，避免相邻窗口连续靠近而把多列串在一起。已有上下窗口的列会进一步吸收中间偏移的单个窗口，但必须有明显横向重叠，且不能与列内窗口明显处于同一行；补入的窗口不会继续吸收其他窗口。最后一项回到第一项，反向导航使用同一顺序的完整逆序。同列中心高度相同时从左到右，中心完全重合时优先按窗口 ID 保持确定顺序。
...
关闭系统的“使窗口按应用程序成组”后，插件会绘制应用图标：48 点图标画布居中压在缩略图底边，添加轻微阴影，不显示应用名称。图标读取应用自身资源，随缩略图移动，退出调度中心或停用插件后移除。开启分组时使用系统原生标识。叠加内容不拦截鼠标。
