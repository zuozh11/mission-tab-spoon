...
所有窗口按**从左到右、从上到下**的逐行顺序循环，走完一行再进入下一行，最后一项回到第一项。按缩略图中心识别行：与该行最上方中心的纵向距离不超过该行与当前缩略图最小高度的一半时视为同行；行内按中心横坐标排序。同列按从上到下访问，位置完全重合时优先按窗口 ID 保持确定顺序。反向导航使用同一顺序的完整逆序。
