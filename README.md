...
当前实验版中，短按和长按 `⌘Tab` 都展开调度中心；按 Tab 选择，松开 Command 确认。MissionTab 是一个 [Hammerspoon](https://www.hammerspoon.org/) Spoon，使用系统调度中心和原生悬停高亮。
...
- **统一进入调度中心**：按下 `⌘Tab` 即请求展开窗口，不再回放短按原生应用切换。
- **支持提前松手**：快速松开 Command 时，等待窗口布局就绪后确认初始目标。
...
| 操作 | 行为 |
| --- | --- |
| 短按 `⌘Tab` | 进入调度中心，布局就绪后确认目标 |
| 按住 Command 继续按 Tab | 保持调度中心并继续选择窗口 |
| Tab | 顺时针选择下一项 |
| 反引号或 Shift+Tab | 反向选择 |
...
| 设置 | 默认值 | 作用 |
| --- | --- | --- |
| `holdDelay` | 兼容旧配置 | 本实验版不使用长短按阈值，此参数不影响触发 |
