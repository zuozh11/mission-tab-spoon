...
- **Tab / 反引号**：应用组之间顺时针 / 逆时针循环；组内 Tab 从上到下，反引号从下到上。Shift+Tab 同反引号。
- **松开 Command**：激活选中窗口。
- **Esc**：取消，返回原窗口。
- **移动鼠标**：改用鼠标选择，松开 Command 仍会退出调度中心；再按 Tab / 反引号可回到键盘导航。
- **调度中心已经打开时按 `⌘Tab`**：直接退出，避免遗留界面无法通过快捷键关闭。

长按选择当前调度中心展示的窗口；不会遍历其他桌面或恢复最小化窗口。初始选择优先为最近使用的其他窗口；同应用、同桌面、同显示器内互相重叠的缩略图作为一个整体，围绕布局中心顺时针排列；组内按屏幕位置从上到下连续访问，走完才进入下一组。未堆叠的窗口独立参与循环。首次用于打开的 Tab 不额外跳过初始窗口。窗口元素出现后立即预定位，动画中跟随其坐标更新；布局稳定后固定本次顺时针顺序。
