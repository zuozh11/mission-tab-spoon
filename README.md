...
通过 `⌘Tab` 进入时，首次导航键选中并高亮最近使用的其他窗口，并将鼠标定位到其缩略图；后续导航键按空间顺序循环。鼠标接管后再按 Tab，若鼠标指向窗口则直接选择下一项；反引号或 Shift+Tab 选择上一项。鼠标没有指向窗口时直接选择当前屏幕内最近的窗口（按鼠标到缩略图边缘的距离判断），后续按键继续循环。鼠标移到另一块屏幕后再按导航键，会重新确定屏幕和起点。
