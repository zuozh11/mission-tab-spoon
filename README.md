...
- **未分组窗口标识**：关闭“使窗口按应用程序成组”时，为每个缩略图补上应用图标和名称，按原生样式居中放在底边；系统手势打开调度中心时也会显示。
...
关闭系统的“使窗口按应用程序成组”后，插件会绘制应用图标和名称：48 点图标画布居中压在缩略图底边，下方使用 13 点系统字体。图标读取应用自身资源，名称读取系统应用名称；随缩略图移动，退出调度中心或停用插件后移除。开启分组时使用系统原生标识。叠加内容不拦截鼠标；未分组布局由 macOS 决定，窗口间距过小时名称可能与相邻缩略图或程序坞重叠。
