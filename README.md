...
所有窗口按**逐列顺序**循环：每列先从上到下，走完后进入右侧下一列，最后一项回到第一项。先按缩略图中心从左到右分列：与当前列所有窗口的共同横向区间仍有重叠时归入该列，否则开始下一列；仅边缘相接不算重叠。这样上下错开的窗口仍可归入同列，也不会因相邻窗口依次重叠而把多列串成一列。列内按中心纵坐标排序，同一高度按从左到右访问，位置完全重合时优先按窗口 ID 保持确定顺序。反向导航使用同一顺序的完整逆序。
