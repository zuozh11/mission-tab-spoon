...
- **焦点窗口起步**：进入时优先选择当前焦点窗口，后续按空间顺序循环。
...
| 场景 | 起点 |
| --- | --- |
| 通过 `⌘Tab` 进入 | 鼠标所在屏幕上的当前焦点窗口；不可用时选择最近使用的其他窗口 |
| 鼠标接管后再次按导航键 | 从鼠标指向的窗口推进一项；Tab 向后，反向键向前 |
| 鼠标接管后，鼠标位于空白处 | 当前屏幕内距离鼠标最近的窗口 |
| 进入时无法匹配焦点或最近窗口 | 最靠近布局左上角的窗口 |

通过 `⌘Tab` 进入时，首次导航键选中并高亮当前焦点窗口，并将鼠标定位到其缩略图；后续导航键按空间顺序循环。鼠标接管后再按 Tab，若鼠标指向窗口则直接选择下一项；反引号或 Shift+Tab 选择上一项。鼠标没有指向窗口时直接选择当前屏幕内最近的窗口（按鼠标到缩略图边缘的距离判断），后续按键继续循环。鼠标移到另一块屏幕后再按导航键，会重新确定屏幕和起点。

所有窗口按**逐列顺序**循环：每列先从上到下，走完后进入右侧下一列，最后一项回到第一项。按缩略图中心识别列：与该列最左侧中心的横向距离不超过该列与当前缩略图最小宽度的一半时视为同列；列内按中心纵坐标排序。同一高度按从左到右访问，位置完全重合时优先按窗口 ID 保持确定顺序。反向导航使用同一顺序的完整逆序。
