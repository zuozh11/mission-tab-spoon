...
通过 `⌘Tab` 进入时，首次导航键选中并高亮起点，但保持鼠标原位；再次按导航键时才将鼠标移到选中的缩略图。鼠标接管后再按 Tab，若鼠标指向窗口则直接选择下一项；反引号或 Shift+Tab 选择上一项。鼠标没有指向窗口时直接选择当前屏幕内最近的窗口（按鼠标到缩略图边缘的距离判断），后续按键继续循环。鼠标移到另一块屏幕后再按导航键，会重新确定屏幕和起点。
...
松开 Command 后不等待布局稳定或悬停刷新，目标可用时立即请求退出并聚焦；退出后再次校正焦点并将光标移到窗口中心。系统动画仍由 macOS 控制。Esc 取消会恢复键盘模式前的位置；如果用户已主动接管鼠标，则保留用户位置。
